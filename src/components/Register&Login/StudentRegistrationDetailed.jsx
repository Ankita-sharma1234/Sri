...
const RegisterDetailed = ({
  ...
  const userData = JSON.parse(sessionStorage.getItem("currentUser"));
    ...
    const dob1 = userData.dob;
    const id = userData._id;
  const formattedDate = new Date(dob1).toLocaleDateString();
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    qualification: Yup.string().required("Qualification is required"),
    passingYear: Yup.string().required("Last Year Passing Year is required"),
    qualificationPercentage: Yup.string().required("Qualifying Exam Percentage is required"),
    nationality: Yup.string().required("Nationality is required"),
    admissionSession: Yup.string().required("Session is required"),
    marksheet12th: Yup.mixed().required("Marksheet is required"),
  });
  ...
  const {
    ...
    onSubmit: async (values) => {
      try {
        ...
        formData.append('file', values.marksheet12th);
  ...
  const admissionSessionOptions = [new Date().getFullYear(), new Date().getFullYear() - 1];

  
  return (
    <>
      <Container fluid className="shadow p-3 mb-3 bg-body rounded" style={{ width: "90%", backgroundColor: "#e8e5d1", marginTop: "20px" }}>
        <Box marginBottom={"20px"} backgroundColor={"#00cdac"} p={4} color="white" mt={8} borderRadius="md" shadow="md">
          Registration Details
        </Box>
        <div style={{ overflow: "hidden" }}>
        <Form onSubmit={handleSubmit}>
